            ...
            for images in image_url:
                ...
                for x in copy_list:
                    if not image.is_hd(x, j):
                        i.remove(x)
                        if fssai_flag != 1 and copy_list.index(x) == len(copy_list) - 1:
                            if j not in not_fssai:
                                not_fssai[j] = {}
                                not_fssai[j]['image_url'] = i
                        ...
                        if clas == 'class1':
                            fssai_flag = 1
                            continue
                        elif fssai_flag != 1 and copy_list.index(x) == len(copy_list) - 1:
                            if j not in not_fssai:
                                not_fssai[j] = {}
                                not_fssai[j]['image_url'] = i  
